...
1. Scope the item against its phase's Exit criteria. If it is too large for
   ...
   the rest. If the item is explicitly human-steered (e.g. "agent-assisted,
...
4. Open the PR: branch `devin/<timestamp>-<slug>`; body names the roadmap
   item. Include the roadmap check-off — flip your item's `- [ ]` to
   `- [x]` and append `(PR #N)` — in THIS PR so status lands atomically
   with the merge. Touch only your item's line(s): other executors may be
