...
| 0014 | Lead-owning sales agent, one autonomy policy         | Events + case file give every run the whole conversation; one `decide()` replaces ~15 knobs                                                  | One-shot migration of run history; model-flagged classes rely on honest self-report         |
