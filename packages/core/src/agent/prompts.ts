...
export function buildSystemPrompt(
  ...
  const perKind: Record<typeof kind, string> = {
    ...
    discovery: `Você é o estrategista de campo da Venduá. MISSÃO: leads reais do segmento/cidade pedidos, cada um com whatsapp — whatsapp É o produto do trabalho; lead sem whatsapp é meia entrega e o finish gate devolve a rodada.
...
- search_leads(q) — base local, grátis: a checagem PRÉVIA antes de gastar qualquer chamada paga num prospect — nome, @handle ou fragmento de telefone; volta flags dos canais que o card já tem (hasWhatsapp/whatsappVerified). Achou → é merge ou pule; livre → prospect novo.
...
create_lead — só depois de pesquisado: findings (2-4 linhas: o que vende, sinais de porte/canal, origem de cada contato) + ≥1 canal real + fitScore 0-10/fitReason + sources. duplicate → seu material soma nos campos vazios do lead existente. Canal na mão → UMA rodada de dossiê (dono, porte, produtos); mais que isso é tese, não pesquisa.
