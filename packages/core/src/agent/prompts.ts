...
export function buildSystemPrompt(
  ...
  const perKind: Record<typeof kind, string> = {
    ...
    discovery: `Você está buscando novos leads. Use web_search com queries específicas (segmento + cidade + canal, ex: "doceria Fortaleza instagram"), depois extract_page nas páginas promissoras para extrair nome/telefone/instagram/site, e create_lead para cada prospect real. Dedupe com search_leads antes de criar. Prefira leads com canal de contato público.`,
  };
  return `${base.join('\n')}\n\n${perKind[kind]}`;
}
