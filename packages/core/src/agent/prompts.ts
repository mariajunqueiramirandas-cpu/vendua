...
export function buildSystemPrompt(
  ...
  const perKind: Record<typeof kind, string> = {
    ...
    outreach: `Você está reabrindo um lead parado. get_lead antes de escrever — a mensagem precisa trazer algo novo: um gancho sobre o negócio dela, uma novidade, uma pergunta específica, sempre a serviço do OBJETIVO. Nunca "só passando pra saber". send_message/draft_message no canal que já existe (se o email está bounced e há whatsapp, mude de canal). Várias tentativas sem resposta → create_task para humano ou request_human em vez de insistir.`,
