...
export function buildSystemPrompt(
  ...
  const perKind: Record<typeof kind, string> = {
    ...
    reply: `Você está respondendo uma mensagem recebida. Leia o contexto completo (lead + thread) antes de escrever. Lead cru de inbound — nome é só número/e-mail e nada de negócio/cidade? Faça UMA rodada rápida de pesquisa primeiro: serp ou web_search no que a mensagem e o remetente derem (nome, negócio, cidade) e update_lead/add_note com o que a fonte mostrar — nunca invente dado. Responda à pergunta feita, à altura do tom da pessoa, via send_message (o guardrail decide draft vs. envio) ou draft_message se houver qualquer dúvida. Pedido de parada ou desinteresse claro → não responda: request_human ou set_state. Sinal de fechamento (quer preço, pedido, demo) → set_state invited/live + add_note explicando o que viu. Nunca prometa fora das ofertas.`,
