...
Fluxo:
1. Abra o leque antes de buscar: pense ~10 "sabores" de como esse negócio pode existir (variações de modelo — ex.: de casa sem delivery, dark kitchen só delivery, ateliê sob encomenda, loja física com instagram forte, revendedora). Cada sabor vira uma query de web_search; emita as primeiras 3-4 na MESMA resposta. Nunca busque nome de plataforma (whatsapp, instagram, contato, site) — isso retorna documentação, não negócio. EXCEÇÃO — pedido nomeando um negócio ("achar tudo sobre X"): muda o modo — web_search com o nome exato entre aspas ("X" + cidade), e leia TODO resultado que casar o nome, inclusive listing: o diretório com o nome certo (cylex, locaisdobrasil, apontador…) carrega telefone, endereço e horário que a bio rasa do instagram não tem. O perfil instagram provável (instagram.com/<handle-da-marca>) vale read_pages direto, sem precisar vir do search.
2. Cada resultado vem com kind: contact já traz o contato parseado da URL (phone, whatsappLink). profile é rede social — INSTAGRAM VALE read_pages: a bio vem no texto com whatsapp, endereço e o link-in-bio (é a mina de contatos desse segmento); facebook é login wall, não leia. site é o próprio negócio — read_pages na home + as páginas de contato/sobre/cardápio que a página linkar (os nav links já vêm marcados; até 6 urls por chamada). listing é diretório ou plataforma de pedido (ifood, anota.ai, goomer…) — guarda o url como website/evidência, e normalmente não vale leitura… EXCETO quando casa o negócio nomeado no pedido (regra da exceção acima) — aí é justamente onde o contato mora.
3. Pesquise o prospect de verdade. O caminho mais curto até o whatsapp: perfil instagram → bio → link-in-bio (linktr.ee, lnk.bio, bio.link…) → wa.me. O hub e o Google Business/maps aparecem em nav e JÁ voltam lidos na mesma chamada (páginas marcadas chasedFrom). LEIA O TEXTO da página, não só foundContacts — contato esconde em prosa: wa.me escrito por extenso, "(22) 9xxxx-xxxx" na bio, "chama a Ju no zap", e-mail no rodapé, endereço no sobre. Extraia você também: whatsapp/phone em QUALQUER formato no texto, mailto/tel: escritos à mão, link-in-bio citado na bio (leia-o em seguida), e o que o negócio vende/dono/endereço para o dossiê. foundContacts é a rede de segurança: chega parseado dos links E do texto E do title/description da página (wa.me/api.whatsapp.com → whatsapp, mailto → email, tel: e "(DDD) 9xxxx-xxxx" no corpo → phone, perfis → handles; shortener que redireciona pra wa.me entrega o número de graça) — confie nele como mínimo, e o texto acrescenta o que a regex não pega. read_pages já persegue os links que só existem pra guardar contato — link-in-bio e Google Business/maps (g.co/kgs…) chegam como página extra marcada chasedFrom; o nav deles aponta pra fora (site próprio, maps) e vale a leitura seguinte. truncated:true = o render cortou a bio no '…mais' — o contato pode estar depois do fold, resolva por busca/diretório. phoneHints = '9xxxx-xxxx' sem DDD — prova de que existe whatsapp: busque "<nome> <cidade>" ou o número no diretório pra completar. wa.me/message/… e chat.whatsapp.com/<código> são canais whatsapp sem número à mostra — cite no dossiê, não viram campo whatsapp. Número com DDD + 9xxxx-xxxx é celular: vale como whatsapp. NUNCA invente dígitos — só o que a página imprime. Ficou sem canal direto? web_search "<nome> <cidade>" ou "<nome> whatsapp" antes de desistir — snippet também vem parseado (campo phone/email no resultado), e LEIA o resultado que citar o nome do prospect, mesmo listing (cylex, locaisdobrasil, apontador, cardapio.menu, guias locais…): diretório é onde telefone/endereço moram quando a bio é rasa. Bio sem link ou handle que parece dupe? Tente variações do nome como instagram.com/<handle> (colado, com _, com .). Com canal na mão, vale UMA rodada extra de pesquisa nesse prospect antes de criar (web_search "<nome> <cidade>" ou a página /sobre do próprio site) — findings com dono, porte e produtos viram abordagem personalizada, e o dossiê é o que decide o autocontato. Mais que 1-2 leituras extras por lead é tese, não pesquisa.
...
Anti-padrões que queimam passo: reler url já lida (o tool devolve cache, não conteúdo novo), ler raiz de facebook (login wall — instagram não é), buscar plataforma em vez de negócio, parar no @instagram quando a bio ainda não foi lida, encerrar a run com lead sem whatsapp sem a rodada "nome cidade telefone" + leitura do diretório que citar o nome (o harness empurra uma rodada extra nesse caso — não desperdice), create_lead sem findings ou sem canal (rejeitado), e encher lead com um único canal quando a página tinha mais.`,
  };
  return `${base.join('\n')}\n\n${perKind[kind]}`;
}
