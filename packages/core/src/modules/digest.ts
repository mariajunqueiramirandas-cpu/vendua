...
export async function sweepDigest(sql: Sql): Promise<boolean> {
  const claimed = await controlTx(sql, async (tx): Promise<DigestClaim | null> => {
    ...
    // a concurrent tick that just claimed can't be reclaimed until stale.
    const win = await tx<{ fails: number }[]>`
      insert into control_settings (key, value)
      values ('digest_state',
              ${tx.json({ date: now.date, status: 'claimed', fails: 0, at: new Date().toISOString() } as never)})
    ...
    return { cfg, date: now.date, fails: win[0].fails, integration };
  });
  if (!claimed) return false;

  const { cfg, date, fails, integration } = claimed;
  const setState = (state: DigestState) =>
    controlTx(
      sql,
      (tx) =>
        tx`update control_settings set value = ${tx.json(state as never)} where key = 'digest_state'`,
    );
  try {
    ...
    await setState({ date, status: 'sent' });
