...
6. Merge gate — you are the approver; the check status is not. Devin
   ...
   - quiet window: after your last push, wait ~15 min and re-read the
     PR once — a new actionable comment sends you back to step 5.
...
- Never merge over failing CI or an unresolved high-severity review
  thread.
- Never exceed the concurrency cap (at most 4 child sessions per run,
  spawned only by the orchestrator — a child never spawns descendants),
